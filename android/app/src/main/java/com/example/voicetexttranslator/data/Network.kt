 ...
 * Gson JSON (de)serialization, producing a ready-to-use [TranslateApi]. The
 * base URL comes from [BuildConfig.BASE_URL], configured per build type.
 */
object Network {
    ...
    /**
     * Creates a [TranslateApi] bound to [baseUrl], which must end with `/` so
     * the relative `api/translate` path resolves correctly.
     */
